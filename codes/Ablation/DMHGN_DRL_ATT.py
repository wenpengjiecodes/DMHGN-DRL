...
class Config:
    ...
    def load_from_json(self, json_path):
        ...
        # 消融实验开关：是否启用 HDGAT 中的双模态注意力机制。
        # True  : 完整模型，执行工序-子批注意力和工人-子批注意力。
        # False : HGSAP-PPO-w/o ATT，跳过上述两个注意力模块，线性嵌入后直接池化。
        # 注意：该开关不同于 use_action_attention，后者仅控制候选动作自注意力。
        self.use_hdg_attention = False
...
class HDGAT(nn.Module):
    ...
    def encode_raw_features(self, raw, device):
        ...
        # 1. 保留原有手动特征提取后的线性嵌入。
        op_embeds = self.op_embed(op_feats) if op_feats.size(0) > 0 else torch.zeros(
            (0, self.config.d_operation), device=device
        )
        batch_embeds = self.batch_embed(batch_feats) if batch_feats.size(0) > 0 else torch.zeros(
            (0, self.config.d_batch), device=device
        )
        worker_embeds = self.worker_embed(worker_feats) if worker_feats.size(0) > 0 else torch.zeros(
            (0, self.config.d_worker), device=device
        )

        # 2. 消融实验控制：是否启用 HDGAT 双模态注意力机制。
        # 完整模型：use_hdg_attention=True
        # HGSAP-PPO-w/o ATT：use_hdg_attention=False，跳过注意力模块，直接池化。
        if getattr(self.config, 'use_hdg_attention', True):
            if op_embeds.size(0) > 0 and batch_embeds.size(0) > 0:
                op_embeds, batch_embeds = self.op_batch_attn(
                    op_embeds, batch_embeds, op_batch_mask
                )
            if worker_embeds.size(0) > 0 and batch_embeds.size(0) > 0:
                worker_embeds, batch_embeds = self.worker_batch_attn(
                    worker_embeds, batch_embeds
                )

        # 3. 保持与完整模型相同的池化方式和输出维度：
        #    5 + d_operation + d_batch + d_worker。
        op_pool = op_embeds.mean(dim=0) if op_embeds.size(0) > 0 else torch.zeros(
            self.config.d_operation, device=device
        )
        batch_pool = batch_embeds.mean(dim=0) if batch_embeds.size(0) > 0 else torch.zeros(
            self.config.d_batch, device=device
        )
        worker_pool = worker_embeds.mean(dim=0) if worker_embeds.size(0) > 0 else torch.zeros(
            self.config.d_worker, device=device
        )
...
class PPOAgent:
    def __init__(self, config: Config):
        ...
        if getattr(config, 'use_hdg_attention', True):
            print("当前运行模式：HGSAP-PPO，启用 HDGAT 双模态注意力机制。")
        else:
            print("当前运行模式：HGSAP-PPO-w/o ATT，已关闭 HDGAT 双模态注意力机制。")
        ...
        # CSV日志记录相关：区分完整模型与 w/o ATT 消融版本，避免日志混淆。
        if getattr(config, 'use_hdg_attention', True):
            self.csv_log_path = "training_log_full.csv"
        else:
            self.csv_log_path = "training_log_wo_att.csv"
...
if __name__ == "__main__":
    train_dir = "../../TestData/train_xiaorong_top_K"
    val_dir = "../../TestData/yanzheng_xiaorong_topk"
    save_dir = "saved_models_wo_att_7_13"
    ...
    # HGSAP-PPO-w/o ATT 必须从头重新训练，避免误加载完整模型或旧消融模型权重。
    latest_path = os.path.join(save_dir, "ppo_agent_episode_best_val.pth")
    if getattr(init_config, 'use_hdg_attention', True):
        if os.path.exists(latest_path):
            print(f"[载入已有模型] {latest_path}")
            agent.load_models(latest_path)
    else:
        print("[从头训练] HGSAP-PPO-w/o ATT 不加载已有模型权重。")
