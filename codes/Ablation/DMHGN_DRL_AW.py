# 训练模型文件
# 修改：动态工件到达逻辑改为基于静态工件完成进度触发（使用测试集中的ready_trigger属性）
# 改进：动态归一化参数、动态最大步数、动态Top-K动作数
...
class Config:
    ...
    def load_from_json(self, json_path):
        ...
        # 消融实验开关：HGSAP-PPO-w/o 3DR-AW
        # 关闭三维奖励向量与状态自适应权重机制，改为固定等权重融合奖励。
        # 完整模型中 _compute_reward 返回 [r_cmax, r_energy, r_fatigue]，
        # 并由 AdaptiveWeightCritic 根据状态生成动态权重；
        # 本消融版本中 _compute_reward 直接返回标量融合奖励。
        self.use_three_dim_reward = False
        self.use_adaptive_weight = False
        self.fixed_reward_weights = (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0)
...
class ReturnCalculator:
    ...
    def compute_returns(self, rewards, values, next_values, dones):
        """
        HGSAP-PPO-w/o 3DR-AW 版本的标量 GAE 回报计算。

        参数:
            rewards: (T,) 固定权重融合后的单目标标量奖励
            values: (T,) Critic 标量价值
            next_values: (T,) 下一状态标量价值
            dones: (T,) 终止标志
        返回:
            returns: (T,) 标量折扣回报
            advantages: (T,) 标量优势
        ...
        last_gae_lam = torch.zeros((), dtype=rewards.dtype, device=rewards.device)
...
class PPOAgent:
    def __init__(self, config: Config):
        ...
        # 消融实验二：去掉三维奖励向量和自适应权重，Critic 改为普通标量价值网络。
        self.critic = CriticNetwork(config).to(self.device)
        self.fixed_reward_weights = tuple(getattr(
            config, 'fixed_reward_weights',
            (1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0)
        ))
        print("当前运行模式：HGSAP-PPO-w/o 3DR-AW，使用固定等权重融合奖励，已关闭三维奖励向量与自适应权重机制。")
        ...
        # CSV日志记录相关
        # 固定权重融合奖励消融实验单独保存日志，避免与完整模型混淆。
        self.csv_log_path = "training_log_wo_3dr_aw.csv"
    ...
    def _generate_valid_actions(self, hdg, episode=None):
        ...
        for op_key in ready_ops:
            ...
            # 启发式组合生成
            workers_by_time = sorted(all_workers, key=lambda w: w.next_available_time)
            combo_earliest = tuple(sorted([w.worker_id for w in workers_by_time[:B_k]]))

            workers_by_fatigue = sorted(all_workers, key=lambda w: w.current_fatigue)
            combo_freshest = tuple(sorted([w.worker_id for w in workers_by_fatigue[:B_k]]))

            candidate_combos = {combo_earliest, combo_freshest}

            for worker_tuple in candidate_combos:
        ...
        top_k_candidates = all_candidates[:self.top_k_actions]

        valid_len = len(top_k_candidates)

        # 4. Padding 至 top_k_actions 长度
        # 使用零向量或复制最后一个动作进行填充，避免影响 Softmax
        if valid_len < self.top_k_actions:
            # 创建虚拟填充数据 (动作特征会是全0，通过Mask屏蔽)
            dummy_action = ((None, 0, set()), 0.0, 0.0)
            num_to_pad = self.top_k_actions - valid_len
            top_k_candidates.extend([dummy_action] * num_to_pad)

        # 5. 批量提取特征
        # 返回的是张量 (top_k, D_action)
        action_features = self._extract_action_features_batch(top_k_candidates, hdg, group_stats)

        # 6. 生成 Mask
        mask = torch.zeros(self.top_k_actions, dtype=torch.float32, device=self.device)
        mask[:valid_len] = 1.0

        # valid_actions 仅包含有效动作，供 _execute_action 使用
        valid_actions = [item[0] for item in top_k_candidates[:valid_len]]
    ...
    def _compute_reward(self, hdg, prev_state, current_state, action):
        """
        HGSAP-PPO-w/o 3DR-AW 消融版本的固定权重融合奖励。

        完整模型返回三维奖励向量 [r_cmax, r_energy, r_fatigue]，并由
        AdaptiveWeightCritic 根据当前状态生成自适应权重。
        本消融版本保留三个奖励分量的计算方式，但不再返回三维奖励向量，
        而是采用固定等权重将三者融合为一个标量奖励：
            r_total = 1/3 * r_cmax + 1/3 * r_energy + 1/3 * r_fatigue
        """
        ...
        return float(r_total), current_state
    ...
    def run_one_episode(self, episode=1, deterministic=False, collect_experience=True):
        ...
        with torch.no_grad():
            while not done:
                ...
                step_reward, _ = self._compute_reward(self.hdg, prev_state, current_state, selected_action)

                # w/o 3DR-AW：Critic 只输出标量状态价值；奖励已在 _compute_reward 中固定权重融合。
                value_scalar = self.critic(global_feature.unsqueeze(0)).view(-1)[0].cpu()
                fixed_weight_vec = list(self.fixed_reward_weights)
                current_episode_weights.append(fixed_weight_vec)
                total_episode_reward += float(step_reward)
        ...
        metrics['weights'] = np.mean(current_episode_weights, axis=0).tolist() if current_episode_weights else list(self.fixed_reward_weights)
    ...
    def update_policy(self, episode=1):
        ...
        for _ in range(self.config.update_epochs):
            ...
            # w/o 3DR-AW：普通 Critic 输出标量价值，不再生成三目标自适应权重。
            current_v = self.critic(global_features).view(-1)
            value_loss = F.mse_loss(current_v, batch['return'].view(-1))
            ...
            scalar_adv = batch['advantage'].view(-1)
            scalar_adv = (scalar_adv - scalar_adv.mean()) / (scalar_adv.std(unbiased=False) + 1e-8)
            ...
            del global_features, current_v, value_loss
    ...
    def _compute_episode_returns(self, episode_experience):
        """根据固定权重融合后的标量奖励计算 PPO 回报和优势。"""
        rewards = torch.tensor(
            [e['reward'] for e in episode_experience],
            dtype=torch.float32,
            device=self.device
        ).view(-1)
        values = torch.stack([e['value'] for e in episode_experience]).to(self.device).view(-1)
        dones = torch.tensor(
            [float(e['done']) for e in episode_experience],
            dtype=torch.float32,
            device=self.device
        ).view(-1)
    ...
    def save_models(self, path="saved_models/", episode_num=None):
        ...
        3. 完整训练过程仍由 csv_log_path 指定的 CSV 文件保存，便于后续画图和论文分析。
        """
        ...
        print(f"  已跳过保存完整 training_history，完整日志请查看 {self.csv_log_path}")
...
if __name__ == "__main__":
    ...
    save_dir = "saved_models_wo_3dr_aw"
    ...
    # HGSAP-PPO-w/o 3DR-AW 与完整模型的 Critic 结构和奖励形式不同，必须从头训练。
    # 因此这里不自动加载已有模型，避免误加载完整模型或其他消融模型权重。
    latest_path = os.path.join(save_dir, "ppo_agent_episode_best_val.pth")
    if os.path.exists(latest_path):
        print(f"[从头训练] 检测到已有模型 {latest_path}，但 w/o 3DR-AW 本次不自动加载。")
    else:
        print("[从头训练] HGSAP-PPO-w/o 3DR-AW 不加载已有模型权重。")
